...
enum
{
    ...
    SPELL_FLAME_ORB                 = 57750, // Flame orb damage (too large amount of damage, using walkaround)
    SPELL_FLAME_ORB_H               = 58937,
    SPELL_SEAR_BEAM                 = 57748,
    SPELL_SEAR_BEAM_H               = 58938,
...
struct MANGOS_DLL_DECL boss_taldaramAI : public ScriptedAI
{
    ...
    {
        if (!pWho || m_creature->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE))
            return;
...
struct MANGOS_DLL_DECL mob_flame_orbAI : public ScriptedAI
{
    mob_flame_orbAI(Creature* pCreature) : ScriptedAI(pCreature)
    {
        ...
        SpellEntry* TempSpell = (SpellEntry*)GetSpellStore()->LookupEntry(SPELL_FLAME_ORB);
        if (TempSpell)
            TempSpell->EffectAmplitude[0] = 700;

        TempSpell = (SpellEntry*)GetSpellStore()->LookupEntry(SPELL_FLAME_ORB_H);
        if (TempSpell)
            TempSpell->EffectAmplitude[0] = 700;
    ...
    uint32 m_uiTimeToDie_Timer;

    void Reset()
    {
        m_uiDespawn_Timer = 13000;
        m_uiCast_Timer = 1000;
        m_uiStartMove_Timer = 3000;
        m_bIsFlying = false;
        m_bTimeToDie = false;
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        if (m_bTimeToDie)
        {
            if (m_uiTimeToDie_Timer <= uiDiff)
                m_creature->ForcedDespawn();
            else
                m_uiTimeToDie_Timer -= uiDiff;
        ...
        if (m_uiDespawn_Timer <= uiDiff)
        {
            ...
            m_bTimeToDie = true;
            m_uiTimeToDie_Timer = 600;
        }
        else
            m_uiDespawn_Timer -= uiDiff;

        if (!m_bIsFlying)
            if (m_uiStartMove_Timer <= uiDiff)
            {
