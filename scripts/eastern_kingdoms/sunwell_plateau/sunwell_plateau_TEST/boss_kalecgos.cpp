...
struct MANGOS_DLL_DECL boss_kalecgosAI : public ScriptedAI
{
    ...
    void CastWildMagic()
    {
        ...
        if (pSpellInfo)
            ...
            {
                if (pTarget && pTarget->IsWithinDistInMap(m_creature, 50.0f))
                {
                    pSpellInfo->EffectImplicitTargetA[0] = TARGET_EFFECT_SELECT;
                    pSpellInfo->EffectImplicitTargetB[0] = 0;
                    m_creature->CastSpell(pTarget, pSpellInfo, true);
...
struct MANGOS_DLL_DECL boss_sathrovarrAI : public ScriptedAI
{
    ...
    void Aggro(Unit* pWho)
    {
        ...
        if (Creature* pKalec = m_creature->SummonCreature(MOB_KALEC, m_creature->GetPositionX() + 10, m_creature->GetPositionY() + 5, m_creature->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 10000))
        {
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        if (!bBanished)
            if (Unit* pSpectral = m_creature->GetMap()->GetUnit(m_uiSpectralGUID))
                if (m_creature->GetHealthPercent() > 1.0f)
                    if (!m_creature->IsInRange(pSpectral, 0.0f, 50.0f, true))
                    {
                        m_creature->GetMap()->CreatureRelocation(m_creature, m_creature->GetPositionX(), m_creature->GetPositionY(), DEMON_REALM_Z, m_creature->GetOrientation());
...
struct MANGOS_DLL_DECL boss_kalecAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        if (!bSathDied)
            if (Unit* pSath = m_creature->GetMap()->GetUnit(m_uiSathGUID))
