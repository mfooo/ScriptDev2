...
struct MANGOS_DLL_DECL boss_janalaiAI : public ScriptedAI
{
    ...
    {
        std::list<Creature*> lTempList;
        GetCreatureListWithEntryInGrid(lTempList, m_creature, MOB_EGG, 100.0f);

        if (lTempList.empty())
            return false;

        for (std::list<Creature*>::iterator i = lTempList.begin(); i != lTempList.end(); ++i)
        {
            if (action == 1)
    ...
    {
        std::list<Creature*> lTempList;
        GetCreatureListWithEntryInGrid(lTempList, m_creature, MOB_FIRE_BOMB, 100.0f);
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if(BombTimer < diff)
        {
            ...
            m_creature->SendMonsterMove(JanalainPos[0][0], JanalainPos[0][1], JanalainPos[0][2], SPLINETYPE_NORMAL, SPLINEFLAG_WALKMODE, 0);
...
struct MANGOS_DLL_DECL mob_janalai_hatcherAI : public ScriptedAI
{
    ...
    bool HatchEggs(uint32 num)
    {
        ...
        for (std::list<Creature*>::iterator i = lTempList.begin(); i != lTempList.end() && num > 0; ++i)
        {
